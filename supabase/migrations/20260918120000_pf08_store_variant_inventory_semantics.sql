-- PF-08B2R3: separate customer variant selection from Store inventory tracking.
-- Additive RPC replacement only; no route, catalog data, or lifecycle changes.

CREATE OR REPLACE FUNCTION public.redeem_moviback_reward(
    p_user_id uuid,
...
BEGIN
    ...
    IF v_reward.fulfillment_type = 'store_product' THEN
        ...
        IF v_reward.requires_store_variant THEN
        ...
        ELSE
            SELECT count(*)
            INTO v_active_stock_count
            FROM public.store_product_stock AS stock
            WHERE stock.product_id = v_product.id
              AND stock.is_active = true;

            IF v_active_stock_count > 0 THEN
                SELECT array_agg(stock.id ORDER BY stock.id)
                INTO v_stock_ids
                FROM public.store_product_stock AS stock
                JOIN public.store_product_colors AS color
                  ON color.id = stock.color_id
                 AND color.product_id = v_product.id
                 AND color.is_active = true
                LEFT JOIN public.store_product_sizes AS size
                  ON size.id = stock.size_id
                 AND size.product_id = v_product.id
                WHERE stock.product_id = v_product.id
                  AND stock.is_active = true
                  AND (
                    (v_active_size_count = 0 AND stock.size_id IS NULL)
                    OR (v_active_size_count > 0 AND stock.size_id IS NOT NULL AND size.is_active = true)
                  );

                v_stock_count := coalesce(array_length(v_stock_ids, 1), 0);
                IF v_stock_count <> v_active_stock_count THEN
                    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'PF08_INVALID_STORE_VARIANT';
                END IF;

                IF v_stock_count > 1 THEN
                    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'PF08_AMBIGUOUS_STORE_VARIANT';
                END IF;
            END IF;

            IF v_active_stock_count = 0
               AND (p_store_color_id IS NOT NULL OR p_store_size_id IS NOT NULL) THEN
                RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'PF08_INVALID_STORE_VARIANT';
            END IF;
        END IF;

        IF coalesce(array_length(v_stock_ids, 1), 0) = 1 THEN
            ...
            IF NOT FOUND
               ...
               OR v_stock.product_id <> v_product.id THEN
                RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'PF08_INVALID_STORE_VARIANT';
            END IF;

            IF v_reward.requires_store_variant
               AND (v_stock.color_id <> p_store_color_id
                    OR v_stock.size_id IS DISTINCT FROM p_store_size_id) THEN
                RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'PF08_INVALID_STORE_VARIANT';
            END IF;

            IF NOT v_reward.requires_store_variant THEN
                IF (p_store_color_id IS NULL AND p_store_size_id IS NOT NULL)
                   OR (p_store_color_id IS NOT NULL
                       AND (v_stock.color_id <> p_store_color_id
                            OR v_stock.size_id IS DISTINCT FROM p_store_size_id)) THEN
                    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'PF08_INVALID_STORE_VARIANT';
                END IF;
                ...
                IF v_stock.size_id IS NOT NULL THEN
                    SELECT id, size_label, is_active
                    INTO v_size
                    FROM public.store_product_sizes
                    WHERE id = v_stock.size_id
                      AND product_id = v_product.id
                    FOR SHARE;
