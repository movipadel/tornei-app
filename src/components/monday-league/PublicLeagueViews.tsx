...
export function PublicLeagueMain() {
  ...
  return <div className={styles.page}><PublicNav /><main className={styles.container}><header className={styles.hero}><div className={styles.eyebrow}>Competizione a squadre</div><h1 className={styles.title}>{data.season.status === "phase2" ? <>Monday League<br />Serie A &amp; Serie B</> : <>Monday<br />League</>}</h1><p className={styles.subtitle}>{data.season.name}: classifica, giornate, risultati e protagonisti in un unico spazio.</p><span className={styles.phaseBadge}>{data.selectedPhase.name}</span></header><PhaseNav phases={data.phases} selectedId={data.selectedPhase.id} onSelect={(id) => void load(id)} /><nav className={styles.tabs} aria-label="Contenuti Monday League">{[["standings", "Classifica", Trophy], ["calendar", "Calendario / Risultati", CalendarDays], ["teams", "Squadre", Users]].map(([key, label, Icon]) => <button type="button" key={String(key)} onClick={() => setTab(key as typeof tab)} className={`${styles.tabButton} ${tab === key ? styles.tabButtonActive : ""}`} aria-pressed={tab === key}><Icon size={16} aria-hidden /> {String(label)}</button>)}</nav>{data.hasProvisionalResults ? <div className={styles.notice}>Classifica provvisoria: include risultati non ancora definitivi.</div> : null}{tab === "standings" ? <StandingsView data={data} /> : tab === "calendar" ? <CalendarView rounds={data.rounds} /> : <TeamsView teams={data.teams} phaseId={data.selectedPhase.id} />}</main></div>;
}
