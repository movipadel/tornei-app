...
type CaptainResultContext = { match_id: string | null; result_id?: string | null; is_home_captain?: boolean; is_away_captain?: boolean; can_submit_result: boolean; can_contest_result: boolean; contest_deadline?: string | null; contest_open?: boolean; result_locked_for_captain?: boolean; effective_status?: "submitted" | "contested" | "confirmed"; submitted_at?: string | null; summary?: string | null; sets?: Array<{ number: number; homeGames: number; awayGames: number }>; contest_submitted?: boolean };
type CaptainContext = { is_captain: true; can_edit_profile: boolean; can_edit_roster: boolean; can_submit_lineup: boolean; lineup_locked: boolean; match_id: string | null; lineup_deadline: string | null; own_lineup: null | { revision: number; players: Array<{ id: string; displayName: string }> }; opponent_lineup: RevealedLineup | null; roster: Array<{ id: string; displayName: string; isCaptain: boolean }>; result: CaptainResultContext | null };
...
function CaptainControls({ team, captain, onSaved }: { team: TeamResponse["team"]; captain: CaptainContext; onSaved: () => void }) {
  ...
  const [resultScores, setResultScores] = useState<Array<{ homeGames: string; awayGames: string }>>([{ homeGames: "", awayGames: "" }, { homeGames: "", awayGames: "" }, { homeGames: "", awayGames: "" }]);
  ...
  const playedSets = resultScores.filter((score) => score.homeGames !== "" && score.awayGames !== "").map((score) => ({ homeGames: Number(score.homeGames), awayGames: Number(score.awayGames) }));
  const legalSet = (score: { homeGames: number; awayGames: number }) => (score.homeGames === 6 && score.awayGames >= 0 && score.awayGames <= 4) || (score.awayGames === 6 && score.homeGames >= 0 && score.homeGames <= 4) || (score.homeGames === 7 && [5, 6].includes(score.awayGames)) || (score.awayGames === 7 && [5, 6].includes(score.homeGames));
  const resultHomeSets = playedSets.filter((score) => score.homeGames > score.awayGames).length;
  const resultAwaySets = playedSets.filter((score) => score.awayGames > score.homeGames).length;
  const validResult = playedSets.every(legalSet) && Math.max(resultHomeSets, resultAwaySets) === 2 && ((playedSets.length === 2 && Math.min(resultHomeSets, resultAwaySets) === 0) || (playedSets.length === 3 && Math.min(resultHomeSets, resultAwaySets) === 1));
  const result = captain.result;
  const resultStatus = result?.effective_status === "contested" ? "Contestato" : result?.effective_status === "confirmed" ? "Definitivo" : "Provvisorio";
  async function submitCaptainResult() { if (!result?.match_id || !validResult) return; if (!window.confirm(`Confermi il risultato ${resultHomeSets}–${resultAwaySets}? Dopo l'invio non potrai modificarlo.`)) return; await jsonAction(`/api/monday-league/matches/${result.match_id}/result`, "POST", { sets: playedSets }, "Risultato inviato"); }
  async function contestCaptainResult() { if (!result?.match_id || !result.result_id || !contestReason.trim()) return toast.error("Inserisci la motivazione della contestazione"); if (!window.confirm("Confermi l'invio della contestazione?")) return; await jsonAction(`/api/monday-league/matches/${result.match_id}/contest`, "POST", { result_id: result.result_id, reason: contestReason }, "Contestazione inviata"); }
  return <section className={`${styles.panel} ${styles.contentPanel}`} style={{ marginBottom: 18 }} aria-label="Azioni capitano"><div className={styles.eyebrow}>Area capitano</div><h2 className={styles.sectionTitle}>Gestisci la tua squadra</h2><div style={{ display: "grid", gap: 20, marginTop: 16 }}><div><strong>Modifica profilo squadra</strong><div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 9 }}><input value={slogan} onChange={(e) => setSlogan(e.target.value)} maxLength={160} placeholder="Slogan" style={fieldStyle} /><button disabled={busy} style={actionStyle} onClick={() => void jsonAction(`/api/monday-league/teams/${team.id}/profile`, "PATCH", { slogan }, "Profilo aggiornato")}>Salva profilo</button><label style={actionStyle}><Upload size={15} /> Logo<input hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => void upload("logo", e.target.files?.[0])} /></label><label style={actionStyle}><Upload size={15} /> Immagine squadra<input hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={(e) => void upload("hero", e.target.files?.[0])} /></label></div></div><div><strong>Gestisci rosa</strong>{captain.can_edit_roster ? <><div style={{ display: "grid", gap: 8, marginTop: 9 }}>{roster.map((player, index) => <div key={player.id} style={{ display: "flex", gap: 8 }}><input value={player.displayName} disabled={player.isCaptain} onChange={(e) => setRoster((items) => items.map((item, i) => i === index ? { ...item, displayName: e.target.value } : item))} style={fieldStyle} />{!player.isCaptain ? <button type="button" style={subtleActionStyle} onClick={() => setRoster((items) => items.filter((_, i) => i !== index))}>Rimuovi</button> : <span className={styles.captain}>Capitano</span>}</div>)}</div><div style={{ display: "flex", gap: 8, marginTop: 9 }}><button type="button" disabled={roster.length >= 4} style={subtleActionStyle} onClick={() => setRoster((items) => [...items, { id: `new-${crypto.randomUUID()}`, displayName: "", isCaptain: false }])}>Aggiungi giocatore</button><button disabled={busy} style={actionStyle} onClick={() => void jsonAction(`/api/monday-league/teams/${team.id}/roster`, "PUT", { roster: roster.map((p) => ({ player_id: p.id.startsWith("new-") ? null : p.id, display_name: p.displayName })) }, "Rosa aggiornata")}>Salva rosa</button></div></> : <p className={styles.muted}>Rosa ufficiale — per modifiche contatta l&apos;organizzazione</p>}</div>{captain.match_id ? <div><strong>Inserisci formazione</strong><p className={styles.muted}>{captain.lineup_deadline ? `Scadenza: ${new Intl.DateTimeFormat("it-IT", { dateStyle: "medium", timeStyle: "short", timeZone: "Europe/Rome" }).format(new Date(captain.lineup_deadline))}` : "Partita da programmare"}</p>{captain.can_submit_lineup ? <><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{captain.roster.map((player) => <button type="button" key={player.id} style={selected.includes(player.id) ? actionStyle : subtleActionStyle} onClick={() => togglePlayer(player.id)}>{player.displayName}</button>)}</div><button disabled={busy || selected.length !== 2} style={{ ...actionStyle, marginTop: 9 }} onClick={() => void jsonAction(`/api/monday-league/matches/${captain.match_id}/lineups`, "POST", { team_id: team.id, player_ids: selected }, "Formazione inviata")}>Conferma i 2 giocatori</button></> : <p className={styles.muted}>{captain.lineup_locked ? "Formazione bloccata" : "Formazione non ancora disponibile"}</p>}<p className={styles.muted}>Avversari: {captain.opponent_lineup ? captain.opponent_lineup.state === "submitted" ? captain.opponent_lineup.players.join(" · ") : captain.opponent_lineup.label : "Formazione riservata fino alla scadenza"}</p></div> : null}{result?.match_id ? <div><strong>Risultato partita</strong>{result.can_submit_result ? <><p className={styles.muted}>Inserisci i set giocati. Il terzo set è facoltativo.</p><div style={{ display: "grid", gap: 8 }}>{resultScores.map((score, index) => <div key={index} style={{ display: "grid", gridTemplateColumns: "70px 80px 16px 80px", gap: 8, alignItems: "center" }}><span>Set {index + 1}</span><input aria-label={`Set ${index + 1} casa`} type="number" min="0" max="7" style={fieldStyle} value={score.homeGames} onChange={(event) => setResultScores((items) => items.map((item, i) => i === index ? { ...item, homeGames: event.target.value } : item))} /><span>–</span><input aria-label={`Set ${index + 1} ospite`} type="number" min="0" max="7" style={fieldStyle} value={score.awayGames} onChange={(event) => setResultScores((items) => items.map((item, i) => i === index ? { ...item, awayGames: event.target.value } : item))} /></div>)}</div><p className={styles.muted}>{validResult ? `Anteprima: ${resultHomeSets}–${resultAwaySets}` : "Inserisci un risultato legale 2–0 o 2–1"}</p><button disabled={busy || !validResult} style={actionStyle} onClick={() => void submitCaptainResult()}>Inserisci risultato</button></> : result.result_id ? <><p><strong>{result.summary}</strong> · {resultStatus}</p><p className={styles.muted}>Risultato inviato · Non modificabile{result.submitted_at ? ` · ${new Intl.DateTimeFormat("it-IT", { dateStyle: "medium", timeStyle: "short", timeZone: "Europe/Rome" }).format(new Date(result.submitted_at))}` : ""}</p>{result.contest_deadline ? <p className={styles.muted}>Termine contestazione: {new Intl.DateTimeFormat("it-IT", { dateStyle: "medium", timeStyle: "short", timeZone: "Europe/Rome" }).format(new Date(result.contest_deadline))}</p> : null}{result.can_contest_result ? <><textarea maxLength={1000} style={fieldStyle} value={contestReason} onChange={(event) => setContestReason(event.target.value)} placeholder="Motivazione obbligatoria" /><button disabled={busy || !contestReason.trim()} style={actionStyle} onClick={() => void contestCaptainResult()}>Contesta risultato</button></> : result.contest_submitted ? <p className={styles.muted}>Contestazione inviata</p> : result.is_away_captain && result.effective_status === "confirmed" ? <p className={styles.muted}>Termine contestazione scaduto</p> : null}</> : null}</div> : null}</div></section>;
}
...
export function PublicLeagueMain() {
  ...
  return <div className={styles.page}><PublicNav /><main className={styles.container}><header className={styles.hero}><div className={styles.eyebrow}>Competizione a squadre</div><h1 className={styles.title}>{data.season.status === "phase2" ? <>Monday League<br />Serie A &amp; Serie B</> : data.season.status === "completed" ? <>Monday League<br />Classifica finale</> : <>Monday<br />League</>}</h1><p className={styles.subtitle}>{data.season.name}: classifica, giornate, risultati e protagonisti in un unico spazio.</p><span className={styles.phaseBadge}>{data.selectedPhase.name}</span></header>{data.season.status === "completed" ? <div className={styles.notice}>Campionato concluso · consulta le classifiche finali di Serie A e Serie B.</div> : null}<PhaseNav phases={data.phases} selectedId={data.selectedPhase.id} onSelect={(id) => void load(id)} /><nav className={styles.tabs} aria-label="Contenuti Monday League">{[["standings", "Classifica", Trophy], ["calendar", "Calendario / Risultati", CalendarDays], ["teams", "Squadre", Users]].map(([key, label, Icon]) => <button type="button" key={String(key)} onClick={() => setTab(key as typeof tab)} className={`${styles.tabButton} ${tab === key ? styles.tabButtonActive : ""}`} aria-pressed={tab === key}><Icon size={16} aria-hidden /> {String(label)}</button>)}</nav>{data.hasProvisionalResults ? <div className={styles.notice}>Classifica provvisoria: include risultati non ancora definitivi.</div> : null}{tab === "standings" ? <StandingsView data={data} /> : tab === "calendar" ? <CalendarView rounds={data.rounds} /> : <TeamsView teams={data.teams} phaseId={data.selectedPhase.id} />}</main></div>;
}
