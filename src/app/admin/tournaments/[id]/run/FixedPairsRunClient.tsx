...
function ensureHomeAway(m: ApiMatchFpMaybeLegacy): UiMatch {
  const home = m.home ?? {
    id: String(m.home_pair_id ?? ""),
    name: String(m.home_name ?? "-"),
  };
  const away = m.away ?? {
    id: String(m.away_pair_id ?? ""),
    name: String(m.away_name ?? "-"),
  };
...
export default function FixedPairsRunClient({
  ...
  const matches: UiMatch[] = useMemo(() => {
    ...
    return raw.map((r) => {
      ...
      const resolvedHome =
        homeName === "-" || homeName.trim() === "" ? pairsNameById.get(homeId) : homeName;
      const resolvedAway =
        awayName === "-" || awayName.trim() === "" ? pairsNameById.get(awayId) : awayName;
  ...
  const canGenerateBracket =
    canHaveBracket && isRunning && allGroupCompleted && !hasBracketAlready;
  ...
  async function reloadFromServer() {
    ...
    setReloading(true);
    try {
      const res = await fetch(`/api/admin/tournaments/${data.tournamentId}/run`, {
        cache: "no-store",
      });
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error((json as any).error || "Errore ricarica");
  ...
  async function generateBracket() {
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error((json as any).error || "Errore generazione tabellone");

      if ((json as any).alreadyGenerated) {
          ...
          <div style={{ display: "grid", gap: 10, justifyItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
              ...
              <Btn
                ...
                onClick={() => patchMatch(m.id, m.patch_url, { reset: true })}
              >
                Azzera
              </Btn>
            </div>
              ...
              return (
                <div key={sn} style={{ display: "grid", gap: 6, justifyItems: "center" }}>
                  <div
                    ...
                    style={{ padding: "2px 12px", background: "#f8fafc", fontWeight: 800 }}
                  >
                    Set {sn}
                  </div>
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            >
              <span
                className="base44-chip"
                style={{ ...statusChipStyle(data.status), padding: "2px 10px" }}
              >
          ...
          <div className="base44-card-inner" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {data.groups.length === 0 ? (
              <div style={{ color: "#64748b" }}>
                Nessun girone salvato (forse formato “solo tabellone”).
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div style={{ padding: 14, display: "grid", gap: 14 }}>
                        ...
                        <div style={{ display: "grid", gap: 10 }}>
                          ...
                          {ms.length === 0 ? (
                            <div style={{ color: "#64748b" }}>
                              Nessuna partita in questo girone.
                            </div>
                          ) : (
                            <div style={{ display: "grid", gap: 12 }}>
                              {ms.map((m) => (
